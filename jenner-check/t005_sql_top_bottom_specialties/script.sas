...
/* ---- From Medicare_study.sas: PROC SQL aggregation of average payment by specialty (top & bottom) ---- */
DATA mips_clean;
    ...
    pri_spec_clean = PUT(pri_spec, $20.);
...
PROC PRINT DATA=top_specialties;
    TITLE "Top Medicare Payments by Provider Specialty (avg, descending)";
RUN;
...
PROC PRINT DATA=bottom_specialties;
    TITLE "Bottom Medicare Payments by Provider Specialty (avg, ascending)";
RUN;
